...
export const createOrder = async (req: Request, res: Response) => {
  const userId = req.userId!;
  const { itemId, quantity, price } = req.body as {
    itemId: number[];
    quantity: number[];
    price: number[];
  };
...
export const incrementQuantityCart = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Item quantity incremented in cart", cartItem });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Error incrementing item quantity in cart", error });
  }
};
...
export const decrementQuantityCart = async (req: Request, res: Response) => {
  ...
  try {
    const cartItem = await decrementQuantityInCartInDb(
      userId,
      itemId,
      quantity,
    );
    ...
    res
      .status(200)
      .json({ message: "Item quantity decremented in cart", cartItem });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Error decrementing item quantity in cart", error });
  }
};
